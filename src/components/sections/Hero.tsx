...
import { ArrowRight, Sparkles, Activity, Shield, Bot, Layout, ArrowUpRight } from 'lucide-react';
...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen pt-32 flex flex-col justify-between overflow-hidden dot-grid" id="hero">
      ...
      <div className="absolute top-1/4 left-10 w-[450px] h-[450px] bg-brand-accent/3 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-10 right-10 w-[550px] h-[550px] bg-brand-accent/5 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="absolute inset-0 z-0 opacity-40 pointer-events-none hidden xl:block select-none">
        ...
        {/* Layer 4: Simple Automation Node Connector Flow (Bottom-Right) */}
        <motion.div
          style={{ x: moveX2, y: moveY1 }}
          animate={{ scale: [1, 1.03, 1] }}
          transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
          className="absolute bottom-32 right-16 bg-brand-card rounded-xl border border-brand-border p-4 shadow-md flex items-center gap-3 rotate-[-5deg] pointer-events-auto z-10"
        >
          <div className="w-7 h-7 rounded-lg bg-orange-50 border border-brand-accent/20 flex items-center justify-center">
            <Layout className="w-4 h-4 text-brand-accent" />
          </div>
          <div className="w-4 h-[1px] border-t-2 border-dashed border-brand-accent" />
      ...
      <div className="max-w-4xl mx-auto px-6 md:px-8 w-full text-center relative z-10 my-auto flex flex-col items-center">
        
        {/* Badge styled exactly like Design HTML */}
        <motion.div
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, ease: 'easeOut' }}
          className="inline-flex items-center px-3 py-1.5 rounded-full bg-brand-card border border-brand-border text-[10px] font-bold tracking-widest text-brand-muted uppercase mb-8"
        >
          <span className="text-brand-accent mr-2">●</span> Blueprint v2.0 Engineering
        </motion.div>
